...
# ── Resources & Methods ───────────────────────────────────────────────────────
# All routes proxy to the single Lambda function via {proxy+} catch-all
...
# Root resource method (handles paths without trailing segment)
...
resource "aws_lambda_permission" "apigw_invoke" {
  statement_id  = "api-gateway-invoke-lambda"
